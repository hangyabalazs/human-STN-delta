...
%GETDATA2ANALYSE creates a struct with details of data to analyse
%   allsessions = getdata2analyse(rootdir,...) creates a struct with the list
%   of directories that contain the data specified by name-value pair
%   arguments.  
%
%   Required inputs:
%     ROOTDIR: root directory with patient folders, with the following
%     path structure: rootdir\PD_Cellbase\patient folders\session folders
%     
...
%           (default: 'allpatients')
%     
%     'side'       tested side (contralateral to the hand used in the task)
...
%           ('stimon' and 'bothcond' only in postoperative settings)

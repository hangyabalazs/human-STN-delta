function [propname_stat, propname_boxstat] = responsesorter_PD(pdcells, resdir, tag, twin, issave,excl_beforeEv,isfig)
...
pause(0.5)
propname_stat = [tag 'xpsth_stat_' num2str(twin)];
...
propname_boxstat =[tag 'xboxstat_' num2str(twin)];
...
%         bwin = [-2.5 -1];   % baseline window for MW-test
        bwin = [-2.5 -1];   % baseline window for MW-test
%         bwin = [-3 0];   % baseline window for MW-test
...
%     end
    ...
    try
        %%
        [binrast, stats] = rasterPSTH(cellid,alignevent,shevent,partition,wn,dt,sigma,bwin,twin,issave,resdir,excl_beforeEv,isfig);
    catch
...
%     end
    ...
    for t=1:NumTrials
        baseline_firing(t) = sum(binrast(t,baseline_inx),'omitnan')/lbwin;
    ...
    if issave&&isfig
        cellidt = regexprep(cellid,'\.','_');
        fnm = fullfile(resdir,[cellidt '_' alignevent '_' partition(2:end) '_boxstat']);
        if ~isempty(excl_beforeEv)
...
function [binrast, stats1] = rasterPSTH(cellid,alignevent,shevent,partition,wn,dt,sigma,bwin,twin,issave,response_resdir,excl_beforeEv,isfig)
...
[~, ~, ~, ~, binrast, stats1] = ...
    ...
    'dt',dt,'sigma',sigma,'parts',partition,'isadaptive',0,'first_event',excl_beforeEv,...
    'maxtrialno',Inf,'baselinewin',bwin,'testwin',twin,'relative_threshold',0.1,'display',isfig); % calculate psth
...
if isfig
    ...
    for iP =  1:NumStats
        ...
        copyobj(Ls,PSTHaxis_handle); set(PSTHaxis_handle,'Xlim', x_lim);
...
if issave
    cellidt = regexprep(cellid,'\.','_');
    fnm = fullfile(response_resdir,[cellidt '_' alignevent '_' partition(2:end)]);
    if ~isempty(excl_beforeEv)
        fnm = [fnm '_ExclBef_' excl_beforeEv];
    end
    if isfig
        saveas(V_handle,[fnm '.jpg']);
        saveas(V_handle,[fnm '.fig']);
